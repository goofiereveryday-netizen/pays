...
class PaySlipPDF(FPDF):
    def header(self):
        ...
        if os.path.exists(logo_path):
            # Page is 210mm wide. Logo is 40mm wide.
            # (210 - 40) / 2 = 85mm for perfect center.
            self.image(logo_path, x=85, y=10, w=40)
            # Move the cursor down so text starts below the logo
            self.set_y(55) 
        else:
            # If no logo file exists, just start at a normal height
            self.set_y(10)

        # Medicare Name and Address (already centered using 'C')
