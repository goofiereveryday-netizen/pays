...
class PaySlipPDF(FPDF):
    def header(self):
        ...
        # Medicare Name and Address (Centered)
...
if submit:
    if not emp_name:
        ...
        # --- NEW: Centered Employee Info Section ---
        # Aligning with the 100mm wide table (start_x = 55)
        start_x = 55
        
        pdf.set_x(start_x)
        pdf.cell(50, 8, f"Pay Date: {now.strftime('%Y/%m/%d')}", 0)
        pdf.cell(50, 8, f"Employee Name: {emp_name}", 0, 1)
        
        pdf.set_x(start_x)
        pdf.cell(50, 8, f"Month: {current_month_str}", 0)
        pdf.cell(50, 8, f"Days Worked: {total_days - absent_days}", 0, 1)
        
        pdf.ln(5) # A bit of space before the table
